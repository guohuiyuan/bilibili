...
func VideoCard2msg(str string) (msg []message.MessageSegment, err error) {
	...
	msg = append(msg, message.Text("av", card.AID))
	msg = append(msg, message.Text("标题：", card.Title, "\n"))
	msg = append(msg, message.Text("UP主：", card.Owner.Name, "\n"))
	msg = append(msg, message.Text("播放：", row(card.Stat.View), "，弹幕", row(card.Stat.Danmaku)))
	msg = append(msg, message.Image(card.Pic))
	msg = append(msg, message.Text("点赞：", row(card.Stat.Like), "，投币：", row(card.Stat.Coin), "\n"))
	msg = append(msg, message.Text("收藏：", row(card.Stat.Favorite), "，分享：", row(card.Stat.Share), "\n"))
